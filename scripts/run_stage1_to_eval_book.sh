...
# End-to-end pipeline for book domain:
...
RUN_NAME="${RUN_NAME:-book_$(date +%Y%m%d_%H%M%S)}"
...
EVAL_DATA="${EVAL_DATA:-/data/uqlinh/reczero/data/book/test.parquet}"
...
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/data/uqlinh/ReasonMerge/eval_results/book/merged_model/${RUN_NAME}_test_reczero_eval}"
...
Example:
  CUDA_VISIBLE_DEVICES=3 RUN_NAME=book_run1 $0
EOF
}
