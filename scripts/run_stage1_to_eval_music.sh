...
# End-to-end pipeline for music domain:
...
# Wait for GPU 3 to have > 15 GB free before starting
WAIT_GPU=3
WAIT_MIN_MB=15360
WAIT_POLL=300
echo "Waiting for GPU $WAIT_GPU to have > $((WAIT_MIN_MB / 1024))GB free (checking every $((WAIT_POLL / 60))min)..."
while true; do
    FREE_MB=$(nvidia-smi --query-gpu=memory.free --format=csv,noheader,nounits -i $WAIT_GPU 2>/dev/null | tr -d ' ')
    if [[ -z "$FREE_MB" ]]; then
        echo "Cannot query GPU $WAIT_GPU, retrying in $((WAIT_POLL / 60))min..."
        sleep $WAIT_POLL
        continue
    fi
    echo "[$(date '+%H:%M:%S')] GPU $WAIT_GPU free: ${FREE_MB}MB / ${WAIT_MIN_MB}MB threshold"
...
DEFAULT_INSTRUCT="/data/uqlinh/tallrec/output-music-fixed/final"
DEFAULT_TARGET="/data/uqlinh/reczero-music-ckpt"
DEFAULT_STAGE1_DATA="/home/uqlinh/RAIN-Merging/data/music/music_reason_correct_direct_fail_full_subset.jsonl"
DEFAULT_STAGE2_DATA="/home/uqlinh/RAIN-Merging/data/music/instruction_calibration_set_from_reason_correct_direct_fail.jsonl"
DEFAULT_OUTPUT="/data/uqlinh/rain_merging/stage1_output_music"

RUN_NAME="${RUN_NAME:-music_$(date +%Y%m%d_%H%M%S)}"
...
STAGE1_TEXTS="${STAGE1_TEXTS:-$DEFAULT_STAGE1_DATA}"
STAGE2_TEXTS="${STAGE2_TEXTS:-$DEFAULT_STAGE2_DATA}"
EVAL_DATA="${EVAL_DATA:-/data/uqlinh/ReasonMerge/data/music/test/test_reczero.parquet}"
...
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/data/uqlinh/ReasonMerge/eval_results/music/merged_model/${RUN_NAME}_test_reczero_eval}"

show_help() {
    cat <<EOF
Usage: $0

Music domain end-to-end merge pipeline.
...
Environment overrides:
  ...
  CUDA_VISIBLE_DEVICES (default: 2)

Example:
  CUDA_VISIBLE_DEVICES=3 RUN_NAME=music_run1 $0
EOF
}
...
print_header "End-to-End Merge Pipeline (Music)"
