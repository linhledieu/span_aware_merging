...
The coefficient is based on responsibility-weighted span preference with
multiplicative tag-Fisher suppression:

C_{l,h}^p = sum_s w_s R_{l,h,s}^p
alpha_raw = alpha_upper * C_{l,h}^p / max_s w_s
alpha = min(alpha_upper, max(0.0, alpha_raw * (1.0 - supp)))
...
# Hardcoded span importance values (JSD-derived)
_SPAN_IMPORTANCE = {"user": 0.013093, "item": 0.012846, "match": 0.030227}


def _compute_weights(formula: str, importance: dict = None) -> tuple:
    """Return (w_user, w_item, w_match) for the given formula.

    paper  : π_s = I_s / ΣI_s,  w_s = (1 - π_s) / Σ(1 - π_s')
    inverse: w_s = (1/I_s) / Σ(1/I_s')

    importance: override dict {"user": float, "item": float, "match": float}.
                Defaults to _SPAN_IMPORTANCE (Yelp values) if not provided.
    """
    I = importance if importance is not None else _SPAN_IMPORTANCE
    if formula == "paper":
        total_I = sum(I.values())
        pi = {s: I[s] / total_I for s in I}
        one_minus = {s: 1.0 - pi[s] for s in I}
        denom = sum(one_minus.values())
        w = {s: one_minus[s] / denom for s in I}
    elif formula == "inverse":
        inv = {s: 1.0 / I[s] for s in I}
        denom = sum(inv.values())
        w = {s: inv[s] / denom for s in I}
    else:
        raise ValueError(f"Unknown weight_formula '{formula}'. Choose 'paper' or 'inverse'.")
    return w["user"], w["item"], w["match"]
...
def _clamp_alpha(value, alpha_upper):
    return min(alpha_upper, max(0.0, value))
...
def main():
    ...
    parser.add_argument("--weight_formula", type=str, default=None,
                        choices=["paper", "inverse"],
                        help="Override span weights: 'paper' or 'inverse'. "
                             "If omitted, uses w_user/w_item/w_match from config.")
    parser.add_argument("--span_importance_json", type=str, default=None,
                        help='JSON string {"user":f,"item":f,"match":f} overriding default '
                             'Yelp JSD importance values when using --weight_formula.')
    ...
    format_supp = {
        proj: torch.load(
            os.path.join(cfg["floors_dir"], f"fisher_supp_tag_{proj}.pt"),
            map_location="cpu",
        ).float()
        for proj in ("q", "k", "v", "o")
    }
    ...
    if args.weight_formula is not None:
        importance_override = json.loads(args.span_importance_json) if args.span_importance_json else None
        w_user, w_item, w_match = _compute_weights(args.weight_formula, importance=importance_override)
        print(f"Weight formula '{args.weight_formula}': "
              f"w_user={w_user:.4f}  w_item={w_item:.4f}  w_match={w_match:.4f}")
    else:
        w_user  = float(cfg["w_user"])
        w_item  = float(cfg["w_item"])
        w_match = float(cfg["w_match"])
        print(f"Weights from config: w_user={w_user:.4f}  w_item={w_item:.4f}  w_match={w_match:.4f}")
    ...
    for proj_type, (num_heads, u_heads) in proj_info.items():
        ...
        if tuple(format_supp[proj_type].shape) != expected_supp_shapes[proj_type]:
            raise ValueError(
                f"fisher_supp_tag_{proj_type}.pt shape {tuple(format_supp[proj_type].shape)} != expected {expected_supp_shapes[proj_type]}"
            )

        alpha = torch.zeros((num_layers, num_heads), dtype=torch.float32)
        supp_t = format_supp[proj_type]
        ...
        for layer in range(num_layers):
            for head_index in range(num_heads):
                ...
                supp_val = supp_t[layer, head_index].item()
                alpha_final = alpha_raw * (1.0 - supp_val)
                alpha_final = _clamp_alpha(alpha_final, alpha_upper)

                if supp_val > 0.0:
                    diagnostics["suppressed_heads"] = diagnostics.get("suppressed_heads", 0) + 1
    ...
    meta_out = {
        "formula_description": (
            ...
            "then multiplicatively suppressed by tag Fisher and clipped to alpha_upper. "
            "Alignment and leakage are not used."
        ),
        "a_align_used": False,
        "u_leak_used": False,
    ...
    print(f"  Heads with nonzero suppression: {diagnostics.get('suppressed_heads', 0)}")
